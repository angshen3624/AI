...
## Date: 06/05/2015
## Description: An improved classifier
...
class HMM:
    ...
    def label( self, data ):
        ...
        ## strokeFeatures (i.e. - data) look like this: 
        ## [{'length': 1}, {'length': 1}, {'length': 1}, {'length': 1},
        ## {'length': 1}, {'length': 1}, {'length': 1}, {'length': 1}, {'length': 0}]

        ## evidence like:
        ## emissions = {'text': {'length': [0.63404825737265413, 0.36595174262734587]}, 
        ## 'drawing': {'length': [0.31615925058548011, 0.68384074941451989]}}

        V = [{}]
        path = {}
        for state in self.states:
            V[0][state] = self.priors[state] * \
                          self.getEmissionProb(state, data[0])
            path[state] = [state]
        ...
        for i in range(1,len(data)):
            ...
            for cur_state in self.states:
                prob, state = max((V[i-1][pre_state] * \
                			  self.getEmissionProb(cur_state, data[i]) * \
                    		  self.transitions[pre_state][cur_state], pre_state) for pre_state in self.states)
                V[i][cur_state] = prob
                new_path[cur_state] = path[state] + [cur_state]

            path = new_path

        prob, s = max((V[i][state], state) for state in self.states)
        ## print prob
    ...
    def simple_test(self):
        """
        simple test from class form website
        """
        self.priors = {'Sunny': 0.63, 'Cloudy': 0.17, 'Rainy': 0.20}
        self.states = ['Sunny', 'Cloudy', 'Rainy']
        self.emissions = {'Sunny': {'length': [0.60, 0.20, 0.15, 0.05]}, \
                          'Cloudy': {'length': [0.25, 0.25, 0.25, 0.25]}, \
                          'Rainy': {'length': [0.05, 0.10, 0.35, 0.50]}}
        self.transitions = {'Sunny': {'Sunny': 0.50, 'Cloudy': 0.250, 'Rainy': 0.250}, \
                            'Cloudy': {'Sunny': 0.375, 'Cloudy': 0.125, 'Rainy': 0.375}, \
                            'Rainy': {'Sunny': 0.125, 'Cloudy': 0.675, 'Rainy': 0.375}}

        dat = [{'length': 0}, {'length': 2}, {'length': 3}]
...
class StrokeLabeler:
    def __init__(self):
        ...
        self.featureNames = ['length', 'side', 'curvature'] ##
        self.contOrDisc = {'length': DISCRETE, 'side': DISCRETE, 'curvature': CONTINUOUS} ##
        self.numFVals = { 'length': 2, 'side': 2} ##
    ...
    def featurefy( self, strokes ):
        ...
        for s in strokes:
            ...
            dist = s.distanceFromSide()
            rt = 0.20
            ran = rt * (right - left)
            if dist < left+ran or dist > right-ran:
            	d['side'] = 0
            else:
            	d['side'] = 1
    ...
    def testHMM( self,testingFiles ):
		...
		for f in testingFiles:
			...
			strokes, truelabels = self.loadLabeledFile(f)
			labels = self.labelStrokes(strokes)
			for tl in truelabels:
				a.append(tl)
			for l in labels:
				b.append(l)
		print self.confusion(a,b)


    def testHMMDir( self,testingDir ):
    ...
    def featureTest( self, strokeFile ):
        ...
        for i in range(len(strokes)):
            ...
            print "Side is", strokes[i].distanceFromSide()
...
class Stroke:
    ...
    def distanceFromSide( self ):
    	'''return the distance form the side'''

    	numOfPoints = float(len(self.points))
    	sum_x = 0.0

    	for p in self.points:
    		sum_x += p[0]

    	dist = sum_x/numOfPoints
